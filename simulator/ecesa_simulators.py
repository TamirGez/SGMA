...
class ECESAGDSimulator(BaseSimulator):
    def __init__(self, sim_param_dict):
        super().__init__(sim_param_dict)
        self.ECESA_P = sim_param_dict["ECESA_P"]
    ...
    def set_models(self):
        ...
        self.server_model.default_dir = 'ECESA_GD'
    ...
    def one_iteration_running(self, time_index, channel_noise=None, receiver_noise=None):
        ...
        for i in range(self.worker_number):
            ...
            thr = np.percentile(channel_noise[i], (1 - self.ECESA_P) * 100)
            if channel_noise[i] >= thr:
                afective_worker_counter += 1
                self.server_model.grad += channel_noise[i] * np.copy(self.workers[i].grad) + receiver_noise[i]

        # final server gradient update
        self.server_model.grad = self.server_model.grad / afective_worker_counter
...
class ECESASGDSimulator(BaseSimulator):
    def __init__(self, sim_param_dict):
        super().__init__(sim_param_dict)
        self.ECESA_P = sim_param_dict["ECESA_P"]
    ...
    def set_models(self):
        ...
        self.server_model = SubGradientDescentModel(self.x_train, self.y_train, self.x_val, self.y_val,
                                                    self.sim_param_dict)
        self.server_model.beta = 1 / (mu_h * self.server_model.L)
        self.server_model.default_dir = 'ECESA_DSGD'
...
class ECESAAGDSimulator(BaseSimulator):
    def __init__(self, sim_param_dict):
        super().__init__(sim_param_dict)
        self.ECESA_P = sim_param_dict["ECESA_P"]
    ...
    def set_models(self):
        ...
        self.server_model = AcceleratedGradientDescentModel(self.x_train, self.y_train, self.x_val, self.y_val,
                                                         self.sim_param_dict)
        self.server_model.beta = 1 / (mu_h * self.server_model.L)
        self.server_model.default_dir = 'ECESA_AGD'
        for i in range(self.worker_number):
